...
function LeaderboardItem({ user, score, rank }) {
  ...
  return (
    <article className="leaderboard-item d-flex mb-2">
      <div
        className={`leaderboard-number align-middle d-flex align-items-center ${setActive()}`}
      >
        <span>{rank}</span>
      </div>
      <div
        className={`leaderboard-body flex-grow-1 d-flex justify-content-between ${setActive()}`}
      >
        <div className="leaderboard-profile d-flex align-items-center">
          <img src={user.avatar} alt={user.name} width="25" className="me-2" />
          <span>{user.name}</span>
        </div>
